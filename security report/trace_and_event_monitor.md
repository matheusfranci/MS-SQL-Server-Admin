1. **Consulta de Traces Ativos**: 
   - Busca as informações dos traces ativos no SQL Server, ignorando o trace específico de 'Duracao.trc' e considerando apenas os traces com status ativo.

2. **Armazenamento dos Resultados de Trace**: 
   - Extrai o ID e o caminho do arquivo de trace específico (`Query_Demorada.trc`) e interrompe esse trace antes de buscar os dados gravados no arquivo de trace.

3. **Interrupção e Exclusão de Trace**: 
   - Executa a interrupção do trace especificado e exclui sua definição do servidor, além de buscar os dados do trace, filtrando por duração e número de leituras.

4. **Apagar Arquivo de Trace**: 
   - Verifica se o comando `xp_cmdshell` está ativado e, caso não esteja, o habilita. Em seguida, executa a exclusão do arquivo de trace gerado anteriormente.

5. **Reativação do Trace**: 
   - Cria e configura um novo trace para monitorar queries lentas, definindo eventos e filtros para capturar dados de execução com base no tempo de duração das queries.

6. **Consulta de Sessões de Extended Events (XE)**: 
   - Recupera informações sobre as sessões de Extended Events (XE) ativas no servidor, filtrando as que não fazem parte de eventos internos do sistema.

7. **Criação de Extended Event para Queries Lentas**: 
   - Cria uma sessão de Extended Event para capturar queries que excedem 3 segundos de duração e armazena os dados em arquivos de evento.

8. **Ativação da Sessão de Extended Events**: 
   - Ativa a sessão de Extended Events criada para monitoramento de queries lentas.

9. **Leitura de Dados de Extended Events**: 
   - Lê os dados armazenados nos arquivos de evento gerados pela sessão de XE e os processa, convertendo os timestamps e extraindo informações relevantes como duração, CPU, leituras e outros dados de execução.
