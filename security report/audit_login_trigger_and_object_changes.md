...
Este script configura dois tipos de triggers (auditoria de logins e auditoria de alterações em objetos de banco de dados) no SQL Server, além de criar uma chave mestra e um certificado de segurança para a instância do servidor.

### 1. **Verificando as Triggers Ativas**
   - A primeira parte do script consulta as triggers de servidor ativas que não são de sistema e estão habilitadas, com a query `SELECT DISTINCT` para mostrar o nome da trigger e o tipo de evento.

### 2. **Criando e Habilitando a Trigger de Auditoria de Logins**
   - A trigger `trgAudit_Login` é criada para auditar logins no servidor. Ela verifica se o login é de um usuário de sistema e impede logins de usuários não permitidos, exibindo uma mensagem e realizando um rollback.
   - A trigger também armazena informações sobre o login (usuario, IP, hostname, software) em uma tabela de auditoria `master.dbo.Logins`.

### 3. **Criando e Habilitando a Trigger de Auditoria de Alterações em Objetos**
   - A trigger `trgAlteracao_Objetos` é criada para auditar eventos DDL (Data Definition Language) no servidor, como alterações em tabelas e objetos do banco de dados.
   - As informações sobre o evento (tipo de evento, usuário, objeto alterado, etc.) são armazenadas na tabela `dbo.Alteracao_Objetos`.
   - A trigger ignora eventos na `tempdb`.

### 4. **Criação de Chave Mestra e Certificado**
   - O script cria uma chave mestra com uma senha forte e, em seguida, cria um certificado com o assunto 'Meu Certificado da Instância', utilizado para garantir a segurança das transações e dados no servidor.
