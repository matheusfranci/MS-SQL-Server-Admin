# **Procedimento Armazenado: `stpChecklist_Seguranca`**

## **Descrição**
O `stpChecklist_Seguranca` é um procedimento armazenado para ambientes SQL Server, desenvolvido para realizar mais de 70 validações de segurança. Ele identifica configurações inadequadas ou vulnerabilidades que podem comprometer a segurança e a integridade do ambiente.

## **Verificações Realizadas**
Abaixo estão as principais verificações realizadas pelo script:

1. Verifica se o banco está marcado como **TRUSTWORTHY**.
2. Confirma se a instância está auditando **falhas de login**.
3. Checa se a instância permite **logins SQL Server**.
4. Verifica a configuração de **Ad hoc distributed queries**.
5. Valida se está habilitado o **cross db ownership chaining**.
6. Identifica se a máquina está aplicando **Windows Updates**.
7. Verifica **databases sem verificação de página** (CHECKSUM).
8. Confirma se o **default trace** está ativado.
9. Checa a configuração de **scan for startup procs**.
10. Verifica se o **DatabaseMail** está habilitado.
11. Valida se o **SQL Mail XP** (antigo DatabaseMail) está ativado.
12. Analisa se o **Remote Admin Connections (DAC)** está habilitado.
13. Confirma a configuração de **Remote Connections**.
14. Checa se o **SMO and DMO XPs** está ativado.
15. Detecta a existência de triggers a nível de servidor.
16. Verifica traces habilitados no servidor.
17. Avalia se há **Extended Events (XE)** habilitados no servidor.
18. Identifica erros no log de **falha de login**.
19. Checa se comandos **xp_cmdshell** estão permitidos.
20. Verifica permissões para **OLE Automation**.
21. Confirma se o **SQLCLR** está habilitado.
22. Avalia se comandos externos estão permitidos na instância.
23. Checa o suporte ao **TDE** (Transparent Data Encryption) nos bancos.
24. Verifica se os **backups dos bancos de dados estão atualizados**.
25. Detecta backups realizados sem **criptografia**.
26. Identifica bancos com **Recovery Model FULL** sem backup de log.

## **Notas**
- **Versão**: 1.0.4 (11/04/2019).
- Desenvolvido por **Fabricio Lima**.
- Contato: [contato@fabriciolima.net](mailto:contato@fabriciolima.net).

## **Uso**
Execute o procedimento para gerar o relatório de segurança:
```sql
