# Principais Pontos do Script de Verificação de Boas Práticas (BPCheck)

## 1. Objetivo
- O script realiza verificações de boas práticas para servidores SQL Server.
- É focado em desempenho, otimização e verificações de configuração.

## 2. Configurações Importantes
- **@duration**: Define o intervalo (em segundos) entre os pontos de coleta de dados sobre contadores de desempenho, esperas e travamentos.
  - Valores entre 10 e 255 segundos (padrão: 90 segundos).
- **@ptochecks**: Controle das verificações voltadas para ajustes de desempenho e otimização.
- **@custompath**: Caminho personalizado para arquivos `.ps1` (scripts PowerShell). 
  - Caso não configurado, o caminho padrão é a pasta de logs.
- **@allow_xpcmdshell**: Ativa ou desativa verificações dependentes da função `xp_cmdshell`.
  - A configuração original da função no servidor permanece inalterada.
- **@spn_check**: Controla a realização de verificações relacionadas a SPN (Service Principal Names).
- **@diskfrag**: Ativa a verificação de fragmentação física de discos.
  - Pode ser demorado em discos grandes e requer privilégios elevados.

## 3. Autor e Referências
- Autor: Pedro Lopes, Microsoft.
- Links relevantes:
  - [Boas Práticas para SQL Server](http://aka.ms/BPCheck)
  - [SQL Insights](http://aka.ms/sqlinsights)

## 4. Notas
- A verificação de fragmentação física de discos pode ser demorada e requer permissões elevadas. Consulte [Documentação sobre Defragmentação](https://support.microsoft.com/en-us/help/3195161/defragmenting-sql-server-database-disk-drives).
...
SET @custompath = 'C:\orion\assessment'
SET @allow_xpcmdshell = 1 --(1 = ON; 0 = OFF)
SET @spn_check = 0 --(1 = ON; 0 = OFF)
SET @diskfrag = 0 --(1 = ON; 0 = OFF)
SET @ixfrag = 1 --(1 = ON; 0 = OFF)
SET @ixfragscanmode = 'SAMPLED' --(Valid inputs are DEFAULT, NULL, LIMITED, SAMPLED, or DETAILED. The default (NULL) is LIMITED)
...
IF (SELECT COUNT(*) FROM sys.servers AS s LEFT JOIN sys.linked_logins AS l (NOLOCK) ON s.server_id = l.server_id LEFT JOIN sys.server_principals AS p (NOLOCK) ON p.principal_id = l.local_principal_id WHERE s.is_linked = 1) > 0
BEGIN
...
LEFT JOIN sys.linked_logins AS l (NOLOCK) ON s.server_id = l.server_id
...
BEGIN
	...
	db.is_trustworthy_on, db.is_db_chaining_on
...
BEGIN
	...
	db.target_recovery_time_in_seconds, db.is_encrypted, db.is_trustworthy_on, db.is_db_chaining_on
FROM master.sys.databases AS db (NOLOCK)
INNER JOIN ##tmpdbsizes AS dbsize (NOLOCK) ON db.database_id = dbsize.database_id
INNER JOIN sys.dm_os_performance_counters AS lu (NOLOCK) ON db.name = lu.instance_name
INNER JOIN sys.dm_os_performance_counters AS ls (NOLOCK) ON db.name = ls.instance_name
...
BEGIN
	...
	BEGIN
		...
		BEGIN -- Scan for local disks
			SET @Text1 = '[string] $serverName = ''WEBDB17''
...
-- IFI subsection
--------------------------------------------------------------------------------------------------------------------------------
RAISERROR (N'  |-Starting IFI instant file initialization', 10, 1) WITH NOWAIT
...
foreach($vol in $vols)
{
	...
	Write-Output $drive
				...
				FROM @output_hw_tot WHERE [PS_OUTPUT] LIKE '%cannot be loaded because%'
					...
					OR [PS_OUTPUT] LIKE '%scripts est d鳡ctiv饥') = 0
				BEGIN
...
BEGIN
	...
	BEGIN	
		...
		BEGIN
			...
			BEGIN
				SET @sqlcmd = 'USE ' + QUOTENAME(@dbname) + ';
select DISTINCT aux.[DatabaseName], aux.[databaseID], aux.objectID, aux.schemaName, aux.tableName, 
	sp.last_updated, sp.[rows], sp.modification_counter, aux.[stats_id], aux.[stat_name], aux.auto_created, aux.user_created, aux.has_filter, aux.filter_definition, sp.unfiltered_rows, sp.steps
FROM (
	SELECT DISTINCT ''' + REPLACE(@dbname, CHAR(39), CHAR(95)) + ''' AS [DatabaseName], ''' + CONVERT(VARCHAR(12),@dbid) + ''' AS [databaseID], mst.[object_id] AS objectID, t.name AS schemaName, OBJECT_NAME(mst.[object_id]) AS tableName, 
		ss.[stats_id], ss.name AS [stat_name], ss.auto_created, ss.user_created, ss.has_filter, ss.filter_definition
	...
	CROSS APPLY sys.dm_db_stats_properties(aux.objectID, aux.[stats_id]) AS sp
WHERE sp.[rows] > 0
		...
		BEGIN		
			...
			BEGIN
				...
				BEGIN
					SET @sqlcmd = 'USE ' + QUOTENAME(@dbname) + ';
SELECT DISTINCT aux.[DatabaseName], aux.[databaseID], aux.objectID, aux.schemaName, aux.tableName, 
	sp.last_updated, sp.[rows], sp.modification_counter,aux.[stats_id], aux.[stat_name], sp.rows_sampled, aux.auto_created, aux.user_created, aux.has_filter,aux.filter_definition, sp.unfiltered_rows, sp.steps
FROM (		
	SELECT  ''' + REPLACE(@dbname, CHAR(39), CHAR(95)) + ''' AS [DatabaseName], ''' + CONVERT(VARCHAR(12),@dbid) + ''' AS [databaseID], mst.[object_id] AS objectID, t.name AS schemaName, OBJECT_NAME(mst.[object_id]) AS tableName, 
		ss.[stats_id], ss.name AS [stat_name], ss.auto_created, ss.user_created, ss.has_filter, ss.filter_definition
	FROM sys.objects AS o
		INNER JOIN sys.tables AS mst ON mst.[object_id] = o.[object_id]
		INNER JOIN sys.schemas AS t ON t.[schema_id] = mst.[schema_id]
